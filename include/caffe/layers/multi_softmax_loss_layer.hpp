#ifndef MULTI_SOFTMAX_WITH_LOSS_LAYER_HPP
#define MULTI_SOFTMAX_WITH_LOSS_LAYER_HPP
...
class MultiSoftmaxWithLossLayer : public LossLayer<Dtype> {
 ...
 protected:
  ...
  void invalidate_probs();
  void create_probs(int count);

  /// The internal SoftmaxLayer used to map predictions to a distribution.
  shared_ptr<Layer<Dtype> > softmax_layer_;
  /// prob stores the output probability predictions from the SoftmaxLayer.
  Blob<Dtype> prob_;
  /// dummy bottom layer for softamx initialisation
  Blob<Dtype> single_softmax_bottom;
  /// bottom vector holder used in call to the underlying SoftmaxLayer::Forward
  vector<Blob<Dtype>*> softmax_bottom_vec_;
  /// top vector holder used in call to the underlying SoftmaxLayer::Forward
  vector<Blob<Dtype>*> softmax_top_vec_;

  int num_classes_, num_classifiers_, softmax_axis_;
};
