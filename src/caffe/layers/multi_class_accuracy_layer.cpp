...
template <typename Dtype>
void MultiClassAccuracyLayer<Dtype>::Forward_cpu(
  ...
  for (int n = 0; n < bottom[0]->num(); ++n) {
    for (int classifier_id = 0;
         classifier_id < num_classifiers_;
         ++classifier_id) {
      const int label_value =
          static_cast<int>(bottom[1]->data_at(n, classifier_id, 0, 0));
      DCHECK_GE(label_value, 0);
      DCHECK_LT(label_value, num_classes_);

      // find element that has the best predicted value and
      // compare its id to label
      const Dtype * begin = bottom_data +
              bottom[0]->offset(n, classifier_id * num_classes_);
      const Dtype * end = bottom_data +
              bottom[0]->offset(n, (classifier_id + 1) * num_classes_);
